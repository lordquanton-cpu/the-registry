# RED ACT — Definitive Product Offering Package

**Classification:** Complete. Client-ready.  
**Version:** 1.0  
**Package Contents:** Emergency Extraction & Crisis Response

---

## Package Structure

| Document | Purpose |
|----------|---------|
| **[01_COMPANY_OVERVIEW](./01_COMPANY_OVERVIEW.md)** | Who we are. What we do. Why we exist. |
| **[02_PRODUCT_CATALOG](./02_PRODUCT_CATALOG.md)** | Full product line. Service tiers. Add-ons. |
| **[03_EVACUATION_PROTOCOL_SPEC](./03_EVACUATION_PROTOCOL_SPEC.md)** | Flagship product — complete technical specification |
| **[04_SERVICE_AGREEMENT](./04_SERVICE_AGREEMENT.md)** | Terms. Conditions. Discretion. |
| **[05_INQUIRY_PROTOCOL](./05_INQUIRY_PROTOCOL.md)** | How to engage. Vetting. Payment. |
| **[06_ONE_PAGER](./06_ONE_PAGER.md)** | Executive summary — single page |

---

## Assets

| Asset | Description |
|-------|-------------|
| `red-act-logo.png` | Primary mark |
| `red-act-hero.png` | Company banner |
| `evacuation-hero.png` | Flagship product hero |
| `inquiry-protocol.png` | Engagement flow |

---

## Company Identity

**Name:** RED ACT  
**Tagline:** *When you have 72 hours. We have a plan.*  
**Sector:** Emergency extraction. Crisis response. Evacuation.

---

*Discretion assured. Payment in advance. No paper trail.*
